...
def runEverything(datafiles, className, testClass, referenceClasses=["Reference"], includeClassifierScores=False, resultname="", features=None):
    if resultname=="":
        ...
        segm = "full" 
        if("word" in datafiles[0]):
            segm = "word"
        elif("phoneme" in datafiles[0]):
            segm = "phoneme"
        resultname += str(segm) + "_" + str(testClass)+"V"+str(referenceClasses)
        found = False
        while(not found):
            if(os.path.exists(resultname+".txt")):
                resultindex += 1
                resultname = "results/rankedFeatures_"+str(segm) + "_" + str(testClass)+"V"+str(referenceClasses)+str(resultindex)
            else:
                found = True
    ...
    if includeClassifierScores:
        ...
        for clas in classifiers:
            ...
            for nr in range(1,len(sortedFeatures)+1):
                ...
                print("Testing top ", nr," features! for "+str(segm)+" "+str(testClass)+" vs. "+str(referenceClasses)+" in "+str(clas))
            ...
            resultfile.write("--------------------------------\n")
            resultfile.write(str(segm)+" "+ str(testClass)+" vs. "+str(referenceClasses))
            resultfile.write("\n"+str(clas)+":\n\n")
...
def prepareData(datafiles, className, testClass, referenceClasses):
    ...
    # # #Outlier detection
    # if 'pitch_max_mean' in list(X.columns):
    #       outlierConditions = [
    #         ((X["pitch_max_mean"] == 0) & (X["pitch_min_mean"] == 0) & (X["pitch_std_mean"] == 0) 
    #         & (X["pitch_mean_mean"] == 0)) , 
    #         (X["dur_mean"] == 0), 
    #         ((X["intensity_max_mean"] == 0) & (X["intensity_min_mean"] == 0) & (X["intensity_min_mean"] == 0) & (X["intensity_std_mean"] == 0 ) &
    #         (X["intensity_mean_mean"] == 0))
    #     ]
    # else:
    #       outlierConditions = [
    #         ((X["pitch_max"] == 0) & (X["pitch_min"] == 0) & (X["pitch_std"] == 0) 
    #         & (X["pitch_mean"] == 0)) , 
    #         (X["dur"] == 0), 
    #         ((X["intensity_max"] == 0) & (X["intensity_min"] == 0) & (X["intensity_min"] == 0) & (X["intensity_std"] == 0 ) &
    #         (X["intensity_mean"] == 0))
    #     ]
    ...
    if 'count' in X.columns: #count is included in the 'mean' files from praat
        listtodrop.append('count')
    if 'index' in X.columns:
        listtodrop.append('index')
    if 'class.1' in X.columns:
        listtodrop.append('class.1')
    ...
    # print("shape of X just before outlier detection = ", X.shape)
   
    # X["outlier"] = np.select(outlierConditions, np.ones(len(outlierConditions)), default =0)
    # lenOutliers = len(X[X.outlier==1])
    # X = X.drop(X[X.outlier == 1].index)
    # X = X.drop(["outlier"], axis=1)
    # X = X.drop(listtodrop, axis=1) #Delete all columns with those labels 

    # X = X.dropna()
    # print("shape of X just after outlier detection = ", X.shape)
